...
class Trainer:
    def __init__(self, model, data, **kwargs):
        ...
        self.optimizer = self.optim_config['optimizer'](params=self.model.parameters(), lr=self.lr)
        ...
        # dataloaders
        self.train_loader = DataLoader(TensorDataset(data['X_train'], data['Y_train']), batch_size=self.batch_size, shuffle=False)
        self.val_loader   = DataLoader(TensorDataset(data['X_val'], data['Y_val']), batch_size=self.batch_size)
    ...
    def train(self):
        for epoch in range(self.n_epochs):
            ...
            if self.verbose and (epoch+1) % self.print_every == 0:
                print(f"(Epoch {self.epoch} / {self.n_epochs}) train r2: {train_r2:.3f}; val r2: {val_r2:.3f}; loss: {loss.item():.3f}")
        
        self.model.load_state_dict(self.best_params)
        
# batch_size = 32
# n_tbins = 10 # 1 sec, 0.1 sec bins
# input_size = 50 # nfeatures
# hidden_size = 100
# x = torch.randn(batch_size, n_tbins, input_size)
# h_prev = torch.zeros(batch_size, hidden_size) # not updating h_prev...
# y = torch.randn(batch_size, n_tbins, hidden_size)

# gru = RateGRU(input_size, hidden_size)
# gru = train_model(gru, x, y, h_prev)