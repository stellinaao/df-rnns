...
def train_one(cfg, x_tr, y_tr, x_val, y_val, device, RateGRU):
    ...
    model = RateGRU(input_size, hidden_dim, output_size, num_layers=num_layers).to(
        device
    )
    ...
    for epoch in range(EPOCHS):
        ...
        with torch.no_grad():
            vl = [
                criterion(model(xb.to(device), h=None)[0], yb.to(device)).item()
        ...
        if (epoch + 1) % PRINT_EVERY == 0:
            print(
                f"    epoch {epoch + 1:4d} | val_r2 {val_r2:.4f} | val_loss {val_loss_hist[-1]:.5f}"
            )
...
def run_sweep(session_label, out_csv, device, RateGRU):
    ...
    for i, cfg in enumerate(configs, 1):
        ...
        rows.append(row)
        print(
            f"    >>> best_metric={result['best_metric']:.4f} @ epoch {result['best_epoch']}"
        )
