...
def get_data(
    learner="early", p_train=0.75, p_val=0.15, p_test=0.1, BWMS=10, s_per_trial=3
):
...
def reshape_XY(X, Y, b_per_trial):
    ...
    X = X[: n_full_trials * b_per_trial, :].reshape(
        n_full_trials, b_per_trial, X.shape[1]
    )
    Y = Y[: n_full_trials * b_per_trial, :].reshape(
        n_full_trials, b_per_trial, Y.shape[1]
    )
...
def add_sigmoid_params(trial_data, session_data, mb_sigms, mf_sigms):
    for i, block_idx in enumerate(session_data["MBblocks"]):
        ...
        trial_data.loc[mask, ["offset", "slope", "lapse"]] = np.tile(
            mb_sigms[i], (mask.sum(), 1)
        )
