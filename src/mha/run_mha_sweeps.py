"""Vanilla MHA sweep runner for early / late DMAT sessions.

Runs one session at a time (--session early|late) so each can be
launched independently in tmux. Results go to separate CSVs and
already-completed configs are automatically skipped.
"""
...
def _filter_attention_configs(configs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
...
def generate_attention_search_configs(
    ...
    configs = _filter_attention_configs(_dict_product(full_grid))
    for cfg in configs:
...
def run_session(
    ...
    arch_grid = {
        ...
        "attention_type": ["full", "causal", "local"],
        "attn_window": [16, 32, 64, 128, 299],
    }
    train_grid = {
        "epochs": [250],
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Vanilla MHA sweep (early or late session)")
    ...
    parser.add_argument("--results-dir", default="results/mha_vanilla")
